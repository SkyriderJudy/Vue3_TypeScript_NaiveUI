...
import addIcon from "@/assets/AddAlt.svg";
...
const renderSuffix = ({ option }: { option: TreeOption }) => {
      ...
      () =>
        h("img", {
          ...
          style: {
            ...
            transition: "opacity 0.2s", // 同步按钮显隐动画
          },
        })
    );
  };
  ...
  return h(
    ...
    option.isParent
      ? [
          createButton("primary", "新增子类别到", addIcon),
          createButton("primary", "编辑", EditSvg),
          createButton("error", "删除", VueIcon),
        ]
      : [
          createButton("primary", "编辑", addIcon),
          createButton("error", "删除", addIcon),
        ]
  );
};

const handleTreeClick = (option: TreeOption) => {
  console.log(option);
};
</script>
